...
const AnniversaryBadge = styled.img`
  width: 100px;
  height: 100px;
  margin-top: 1rem;
  object-fit: contain;
...
const Footer = () => {
  ...
  const [anniversaryBadge, setAnniversaryBadge] = useState(null);

  useEffect(() => {
    // Load main categories for Quick Links and Featured Services
    const loadCategories = async () => {
      try {
        const { allCategories, logos } = await fetchFrontendData();
        ...
        // Find anniversary badge from logos
        const badge = (logos || []).find(l => l.logo_type === 'anniversary_badge' && l.is_active);
        if (badge) {
          setAnniversaryBadge(badge);
        }
    ...
    <FooterContainer>
      <FooterContent>
        ...
        >
          ...
          {anniversaryBadge && anniversaryBadge.image && (
            <AnniversaryBadge 
              src={anniversaryBadge.image} 
              alt={anniversaryBadge.title || '19 Years Anniversary'} 
            />
          )}
          <SocialLinks>
